...
if __name__ == "__main__":
    ...
    print("\n=================")
    print("\n=== TOTAL NET (no discounting) ===")
    print(f"Baseline (fixed ships): net=${base_total_net:,.0f}")
    print(f"Scenario 1 (ships +10–20%): net=${s1_total_net:,.0f}")
    print(f"Scenario 2 (delayed load 2–5): net=${s2_total_net:,.0f}")
    print(f"Scenario 3 (growth + delayed): net=${s3_total_net:,.0f}")

    # -----------------------------
    # Plot cumulative net cash flow (investment year 1)
    # -----------------------------
    plot_cumulative_net_year1_investment(yearly_base, WHARF_COST, "Baseline: Cumulative net cash flow (investment in Year 1)")
    plot_cumulative_net_year1_investment(yearly_s1,   WHARF_COST, "Scenario 1: Cumulative net cash flow (ships +10–20%)")
    plot_cumulative_net_year1_investment(yearly_s2,   WHARF_COST, "Scenario 2: Cumulative net cash flow (loading delayed 2–5)")
    plot_cumulative_net_year1_investment(yearly_s3,   WHARF_COST, "Scenario 3: Cumulative net cash flow (growth + delayed)")
...
avg_costs = [avg_cost_pre, avg_cost_base, avg_cost_s1, avg_cost_s2, avg_cost_s3]

plot_5yr_avg_cost_barchart(
    avg_costs,
    labels_cost,
    "5-Year Average Annual Demurrage Cost (No Baseline Comparison)"
)

print("\n--- 5-year Average Annual Cost (no comparison) ---")
print(f"Pre-contract (2W,115,2–3): ${avg_cost_pre:,.0f}")
print(f"Baseline (3W):            ${avg_cost_base:,.0f}")
print(f"Scenario 1 (3W,+10–20%):  ${avg_cost_s1:,.0f}")
print(f"Scenario 2 (3W,2–5):      ${avg_cost_s2:,.0f}")
print(f"Scenario 3 (3W,+10–20%,2–5): ${avg_cost_s3:,.0f}")
...
plot_cost_distributions(
     costs_base, costs_s1, costs_s2, costs_s3,
    "Annual Demurrage Cost Distribution (Pre-contract + 3-wharf scenarios)"
)
