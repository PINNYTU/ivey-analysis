...
Runnable in BOTH:
- Terminal / CLI
- Jupyter notebooks (ipykernel passes extra args like -f kernel.json)

Key fix:
- argparse.parse_args() -> argparse.parse_known_args()
"""
...
from Baker_model_Team3 import BakeryDay
...
def simulate_island_day(config: Optional[Dict[str, Any]] = None,
    ...
    if circular:
        ...
        basil_needed = float(b.get("basil_kg_needed", b.get("basil_needed_kg", 0.0)))
        ...
        total_waste_tax = bakery_heat_tax
...
# ══════════════════════════════════════════════════════════════════════════
#  OPTION A — Shared-Environment (Real-time grid exchange)
# ══════════════════════════════════════════════════════════════════════════
...
def simulate_island_day_realtime(config: Optional[Dict[str, Any]] = None,
    ...
    return {
        ...
        "fish_farm": f,
        "greenhouse": g,
        "grid_energy_level": float(grid.energy.level),
        "grid_water_level": float(grid.water.level),
        "grid_produce_level": float(grid.produce.level),
...
def island_monte_carlo(config: Optional[Dict[str, Any]] = None,
                       n_trials: int = 500,
                       circular: bool = True) -> Dict[str, Any]:
    results = [simulate_island_day(config, seed=i, circular=circular) for i in range(n_trials)]
...
def island_grid_search(n_trials: int = 500, verbose: bool = True) -> list:
    ...
    combos = [(o, t, u) for o in oven_range for t in tank_range for u in unit_range]
    summaries = []
...
def _default_config(config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    defaults = dict(
        phase="PHASE_2",
        num_ovens=3, bakery_stagger=15, bakery_demand=400,
        use_grid_basil=True,
...
def run_cli(argv: Optional[Sequence[str]] = None):
    """
    argv:
      - None (default): use sys.argv (terminal)
      - []: notebook-friendly (no extra ipykernel args)
      - custom list: for programmatic calls
    """
    ...
    # ✅ KEY FIX: ignore unknown args (e.g., Jupyter's -f kernel.json)
    ...
    if args.search:
        ...
        _print_day_summary(cfg, seed=args.seed, circular=(not args.standalone))
