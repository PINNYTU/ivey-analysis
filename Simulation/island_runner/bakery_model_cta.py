...
class BakeryDay(EnterpriseDay):
    ...
    def __init__(self, config, env=None, grid=None, prices=None):
        ...
        self._stagger = config.get("bakery_stagger", 20)
...
# ─────────────────────────────────────────────────────────────
# Monte Carlo (Bakery only) — 500 trials
# ─────────────────────────────────────────────────────────────
if __name__ == "__main__":
    import numpy as np
    from island_grid_B import PHASE_2_PRICES, CTA_DIRECTIVES

    # Use CTA cap for ovens (3:1 oven:baker)
    cfg = {
        "num_ovens": int(CTA_DIRECTIVES.get("MAX_OVENS_PER_BAKER", 3)),
        "bakery_stagger": 20,     # your current setting
        "bakery_demand": 400,
        "use_grid_basil": True,   # standalone bakery model treats this as $0 basil
    }
    ...
    for seed in range(N):
        ...
        r = bakery.simulate(seed=seed)
        results.append(r)

    # Pull key metrics
    profit = np.array([r["profit"] for r in results], dtype=float)
    sl = np.array([r["service_level"] for r in results], dtype=float)
    failed = np.array([r["batches_failed"] for r in results], dtype=float)
    completed = np.array([r["batches_completed"] for r in results], dtype=float)
    util = np.array([r["baker_util"] for r in results], dtype=float)
    ...
    print("\n=== Bakery Monte Carlo (N=500) — Standalone ===")
    print(f"Ovens: {cfg['num_ovens']} | Stagger: {cfg['bakery_stagger']} min | Demand mean: {cfg['bakery_demand']}")
    print("\nProfit ($):", summarize(profit))
    print("Service level (%):", summarize(sl))
    print("Batches failed:", summarize(failed))
    print("Batches completed:", summarize(completed))
    print("Baker utilization (%):", summarize(util))

    # Extra “how often we hit targets”
    print("\n--- Reliability checks ---")
    print(f"SL ≥ 90%: {float(np.mean(sl >= 90) * 100):.1f}% of trials")
    print(f"No failed batches: {float(np.mean(failed == 0) * 100):.1f}% of trials")
